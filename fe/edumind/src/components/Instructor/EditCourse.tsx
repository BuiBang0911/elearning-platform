...
import { BookOpen, Download, Edit, Eye, FileText, GripVertical, Plus, Trash2, Upload } from "lucide-react";
...
const EditCourse = ({ editCourseOpen, setEditCourseOpen, selectedCourse, handleMaterialsChanged }: EditCourseProps) => {
	...
	useEffect(() => {
		const fetchCourseDetails = async () => {
			try {
				...
				const documents = await CourseApi.searchDocuments(selectedCourse.id); // Fetch all documents without search term
	...
	const handleDeleteDocument = async (docId: number) => {
		try {
			...
			setCourseDocuments(courseDocuments.filter(d => d.id !== docId));
			handleMaterialsChanged(-1);
		} catch (error) {
					...
					<TabsContent value="basic" className="space-y-4">
						<form className="space-y-4" onSubmit={handleSaveEdit}>
							...
							<div className="flex justify-end gap-2 pt-4">
								<Button type="button" variant="outline" onClick={() => setEditCourseOpen(false)}>
									Cancel
								</Button>
